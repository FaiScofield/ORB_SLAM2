...
void Tracking::Track()
{
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        // System is initialized. Track Frame. 步骤2：跟踪
        ...
        if (!mbOnlyTracking) {
            ...
            if (mState == OK) {
                ...
                // 对于Stereo和RGBD,UpdateLastFrame()函数会为上一帧添加MPs
                // 上一帧的MPs主要是用来在searchByProjection()时加快匹配速率用的
        ...
        if (bOK) {
            ...
            for (auto lit = mlpTemporalPoints.begin(), lend = mlpTemporalPoints.end(); lit != lend;
                 lit++) {
                MapPoint* pMP = *lit;
                delete pMP;
            }
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // 步骤4:通过优化3D-2D的重投影误差来获得当前帧位姿，4次优化，MPs fixed
    // mCurrentFrame.mvbOutlier值会对应改变
    ...
    for (int i = 0; i < mCurrentFrame.N; i++) {
        if (mCurrentFrame.mvpMapPoints[i]) {
            if (mCurrentFrame.mvbOutlier[i]) {
                ...
                mCurrentFrame.mvpMapPoints[i] = static_cast<MapPoint*>(NULL);
                mCurrentFrame.mvbOutlier[i] = false;  // 为何将局外点标志置为false?
...
bool Tracking::TrackWithMotionModel()
{
    ...
    // Update last frame pose according to its reference keyframe Create "visual odometry" points
...
void Tracking::UpdateLocalPoints()
{
    ...
    for (auto itKF = mvpLocalKeyFrames.begin(), itEndKF = mvpLocalKeyFrames.end(); itKF != itEndKF;
         itKF++) {
        KeyFrame* pKF = *itKF;
