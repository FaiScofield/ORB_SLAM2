...
const int ORBmatcher::TH_LOW = 50;  // 匹配距离（误差）阈值
const int ORBmatcher::HISTO_LENGTH = 30;    // 30°一组统计匹配特征点的旋转主方向，如果旋转方向不在最高的3个直方图里，则认为是误匹配
...
int ORBmatcher::SearchByProjection(Frame &F, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for (size_t iMP = 0; iMP < vpMapPoints.size(); iMP++) {
        ...
        // 找出附近的兴趣点
        const vector<size_t> vIndices = F.GetFeaturesInArea(pMP->mTrackProjX, pMP->mTrackProjY,
                                       ...
                                       const cv::Mat &F12, const KeyFrame *pKF2)
{
    ...
    // 求出kp1在pKF2上对应的极线
    const float a =
        kp1.pt.x * F12.at<float>(0, 0) + kp1.pt.y * F12.at<float>(1, 0) + F12.at<float>(2, 0);
    const float b =
        kp1.pt.x * F12.at<float>(0, 1) + kp1.pt.y * F12.at<float>(1, 1) + F12.at<float>(2, 1);
    const float c =
        kp1.pt.x * F12.at<float>(0, 2) + kp1.pt.y * F12.at<float>(1, 2) + F12.at<float>(2, 2);
 ...
 * @param  vpMapPointMatches F中MapPoints对应的KF的匹配，NULL表示未匹配
 * @return                   成功匹配的数量
 */
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    for (int i = 0; i < nMPs; i++) {
        ...
        // Depth must be positive
        // MP深度值必须为正，如果不满足，不应该把它设置为bad，应为它可能在其他KF下深度是正的
        ...
        if (bestDist <= TH_LOW) { // 找到了MapPoint在该区域最佳匹配的特征点
            ...
            if (pMPinKF) { // 如果这个点有对应的MapPoint，替换掉
                if (!pMPinKF->isBad()) { // 如果这个MapPoint不是bad，选择哪一个呢？根据观测数量决定
                    if (pMPinKF->Observations() > pMP->Observations())
                        pMP->Replace(pMPinKF);
                    else
                        pMPinKF->Replace(pMP);
                }
            } else { // 如果这个点没有对应的MapPoint，给它对应上MP属性
                                   ...
                                   const bool bMono)
{
    ...
    for (int i = 0; i < LastFrame.N; i++) {
        ...
        if (pMP) {
            if (!LastFrame.mvbOutlier[i]) { // 对上一帧有效的MapPoints进行跟踪
                // Project
