...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB)
    : mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS),
      ...
      mnFuseTargetForKF(0), mnBALocalForKF(0), mnBAFixedForKF(0), mnLoopQuery(0), mnLoopWords(0),
      mnRelocQuery(0), mnRelocWords(0), mnBAGlobalForKF(0), fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy),
      invfx(F.invfx), invfy(F.invfy), mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N),
      ...
      mpKeyFrameDB(pKFDB), mpORBvocabulary(F.mpORBvocabulary), mbFirstConnection(true),
      mpParent(NULL), mbNotErase(false), mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb / 2),
      mpMap(pMap)
{
    mnId = nNextId++;
...
{
    {
        ...
        else if (mConnectedKeyFrameWeights[pKF] != weight)  // 之前连接的权重不一样，更新一下
            mConnectedKeyFrameWeights[pKF] = weight;
        else
            return;
    }
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    // 按照权重进行排序，从小到大，最后要从大到小存到两个成员变量里
...
vector<KeyFrame *> KeyFrame::GetCovisiblesByWeight(const int &w)
{
    ...
    //!@Vance: 这里用upper_bound()没错，返回大于等于权w的KFs，如果是lower_bound就只返回大于权w的KFs
 ...
 * 3. 对这些连接按照权重从大到小进行排序，以方便将来的处理，更新完covisibility图之后，如果没有初始化过，
